...
instance Show Puzzle where
  show (Puzzle _ discovered guessed) = (intersperse ' ' . map renderPuzzleChar $ discovered) ++ " Guessed so far: " ++ guessed

renderPuzzleChar :: Maybe Char -> Char
renderPuzzleChar Nothing = '_'
renderPuzzleChar (Just l) = l
...
fillInCharacter (Puzzle word filledInSoFar s) c = Puzzle word newFilledInSoFar (c:s)
  where zipper guessed wordChar discovered =
          if guessed == wordChar then Just wordChar else discovered
...
runGame puzzle = forever $ do
  ...
  putStr "Guess a latter: "
  guess <- getLine
  case guess of
    [c] -> handleGuess puzzle c >>= runGame
    _ -> putStrLn "Your guess must be a single characer"
